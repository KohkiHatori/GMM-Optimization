...
mkdir -p "$OUT_DIR"
CSV_OUT="$OUT_DIR/openmp_t${THREADS}.csv"
...
for D in "${D_ARRAY[@]}"; do
    for K in "${K_ARRAY[@]}"; do
        echo "========================================"
        echo "GRID: N=$N, D=$D, K=$K (OpenMP $THREADS threads)"
        echo "========================================"
        ...
        # 2. Run GMM
        echo "-> Running..."
        TEMP_LOG="/tmp/grid_omp_$$.log"
        ./bin/gmm_omp --data "$DATA_BIN" --init "$INIT_BIN" --n $N --dim $D --k $K --iters $ITERS --fit_tol $FIT_TOL > "$TEMP_LOG"
