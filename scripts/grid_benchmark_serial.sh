...
for D in "${D_ARRAY[@]}"; do
    for K in "${K_ARRAY[@]}"; do
        ...
        # 2. Run GMM
        echo "-> Running..."
        TEMP_LOG="/tmp/grid_serial_$$.log"
        ./bin/gmm_serial --data "$DATA_BIN" --init "$INIT_BIN" --n $N --dim $D --k $K --iters $ITERS --fit_tol $FIT_TOL > "$TEMP_LOG"
