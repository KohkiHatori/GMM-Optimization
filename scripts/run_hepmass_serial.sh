...
CSV_OUT="results/timing/hepmass_serial.csv"
...
echo "========================================"
echo "Testing HEPMASS Data (N = $N, D = $DIM, K = $K)"
echo "========================================"

OUT_BIN="results/hepmass_serial_out.bin"

# Run Serial GMM & Capture Output
echo "-> Running Serial GMM..."
TEMP_LOG="/tmp/temp_hepmass_serial_$$.log"
./bin/gmm_serial --data "$DATA_BIN" --init "$INIT_BIN" --n $N --dim $DIM --k $K --iters $ITERS --fit_tol $FIT_TOL --out "$OUT_BIN" > "$TEMP_LOG"
