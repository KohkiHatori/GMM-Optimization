...
int main(int argc, char** argv) {
    ...
    // Report how many threads OpenMP will use
    ...
    gmm_model* model = gmm_init(K, D);

    struct timespec time_start, time_stop;

    printf("Starting GMM Training (N=%d, D=%d, K=%d, Iters=%d)...\n\n", N, D, K, max_iters);
    clock_gettime(CLOCK_REALTIME, &time_start);

    // Parse Initialization Means if Provided
    ...
    // Start EM Algorithm
    gmm_train(data, N, D, K, max_iters, tol, init_means, model);
